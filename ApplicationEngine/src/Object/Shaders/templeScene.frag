...
#define ZERO 0
    ...
    #define AA 1
#else
    #define AA 2   // Set AA to 2 or 3 for antialiasing.
#endif
...
// sdRhombus: Returns the SDF for a rhombus prism.
// la, lb: semi-axes; h: height; ra: corner rounding.
float sdRhombus(in vec3 p, float la, float lb, float h, float ra) {
    p = abs(p);
    vec2 b = vec2(la, lb);
...
// sdPyramid: Returns the SDF for a pyramid with height h.
float sdPyramid(in vec3 p, in float h) {
    ...
    float d2 = (q.y < -q.x * m2 - q.y * 0.5) ? 0.0 : min(a, b);
    return sqrt((d2 + q.z * q.z) / m2) * sign(max(q.z, -p.y));
}
// Overloaded sdCappedCone: using two points (a, b) and radii ra, rb.
...
// sdOctahedron: Returns the SDF for an octahedron with size s.
...
vec2 map(in vec3 pos) {
    ...
    if (sdBox(pos - vec3(0.0, 0.3, -1.0), vec3(0.35, 0.3, 2.5)) < res.x) {
        ...
        // Using sdSolidAngle function if defined...
    }
...
float calcSoftshadow(in vec3 ro, in vec3 rd, in float mint, in float tmax) {
    ...
    return clamp(res, 0.0, 1.0) * clamp(res, 0.0, 1.0) * (3.0 - 2.0 * res);
}
...
vec4 mainImage(in vec2 fragCoord) {
    // Use CurrentPosition as fragment coordinate; Dimensions is our screen size.
    // Set a default mouse coordinate (since iMouse is not available).
    vec2 mo = vec2(0.5, 0.5);
    float t = 32.0 + Time * 1.5;
    
    // Camera setup.
    vec3 ta = vec3(0.25, -0.75, -0.75);
    vec3 ro = ta + vec3(4.5 * cos(0.1 * t + 7.0 * mo.x), 2.2, 4.5 * sin(0.1 * t + 7.0 * mo.x));
    ...
    for(int n = ZERO; n < AA; n++) {
        ...
        vec2 p = (2.0 * (fragCoord + o) - Dimensions) / Dimensions.y;
#else
        vec2 p = (2.0 * fragCoord - Dimensions) / Dimensions.y;
#endif

        const float fl = 2.5;
        vec3 rd = ca * normalize(vec3(p, fl));
        vec2 px = (2.0 * (fragCoord + vec2(1.0, 0.0)) - Dimensions) / Dimensions.y;
        vec2 py = (2.0 * (fragCoord + vec2(0.0, 1.0)) - Dimensions) / Dimensions.y;
...
void main() {
    // Use CurrentPosition as our fragment coordinate.
    FragColor = mainImage(CurrentPosition);
}
